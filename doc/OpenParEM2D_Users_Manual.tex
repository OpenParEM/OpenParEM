...
The goal is that OpenParEM2D always detects problems and exits gracefully with an error message with the lock file removed.  Any other exit style is an issue that needs to be fixed with improved error checking, with one exception.
...
Order ramping is supported where the problem is first solved using \texttt{N}=1, then the solution is used as the initial guess for a second solution with \texttt{N}=2, which then becomes the initial guess for \texttt{N}=3, and so on.  The technique takes advantage of the ability of first-order elements to solve problems with lower quality meshes, then the good initial guesses enable the higher-order elements to solve far more quickly.  Experience has shown order ramping to be very robust, but it can be turned off, if needed.
...
\item Harder convergence for the initial solution if order ramping is disabled.
\end{itemize}
...
\begin{longtable}[c]{|lccp{8cm}|}
    ...
    solution.initial.guess.level  & int    & 2      & Sets the initial guess for the eigensolveer 

                                                      Options:
                                                      \begin{itemize}[nosep]
                                                        \item 0 - use no initial guess or order ramping
                                                        \item 1 - use prior solution as initial guess with frequency scaling (method used in version 2.1 and earlier)
                                                        \item 2 - level 1 plus plus order ramping on the first iteration (default)
                                                        \item 3 - use order ramping for every solution without using any prior solution
                                                      \end{itemize}\\
