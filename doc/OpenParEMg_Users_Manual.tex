...
On startup, OpenParEMg checks for a file called \texttt{.OpenParEMg} in the user's home directory.  If one is not found, one is created using default parameters.  If one is found, then it is read and the values defined there are assigned.  OpenParEMg does not provide any way to edit this file, but since it is a text file, it can be modified by any available text editor.  Any changes are applied at the next start of OpenParEMg.

The variables in \texttt{.OpenParEMg} allow the setting of the overall GUI font and the font in the logging tabs, the main window height and width, the main window origin, and the default number of cores to use in simulations.  To reset the settings to the defaults, simply delete \texttt{.OpenParEMg}, and the file will be regenerated with the default settings at the next start up of OpenParEMg.
...
The \texttt{Log} tab shows the contents the file \texttt{projectname.log} produced by OpenParEM3D while running.  This is the main window to check the current status of the simulation at a given frequency.  Note that running OpenParEM3D at the command line scrolls logging information to the terminal, but when being run by OpenParEMg, the logging information is saved to the log file instead with no output going to the terminal.
...
\newpage
\section{Macro File Format Specification}
\label{sec:macro_file_spec}

A simple macro language is defined to enable stress and regression testing.  The stress and regression tests can be found in \texttt{regression/OpenParEMg}. Given its limited scope, the language is not fully developed and has limited error detection and messaging. Any system implementing full automation of OpenParEM3D is expected instead to use a full-featured programming language to write the necessary text files to develop complete projects.

While a macro is running, the interface is "live", so clicking in the GUI can cause the behavior to change or even for the macro to fail.  For example, if a macro selects two objects for a merge operation, then a click in the drawing area can clear the selection and cause the merge operation to fail, leading the macro to exit with an error message.

\begin{Verbatim}[fontsize=\small]
#OpenParEMgMacro 1.0 // required in the file
...
# Close project that has no pending changes
closeProject

# Close project without saving 
forceCloseProject

# Set the drawing plane with the origin at (x0,y0,z0), the u-direction (ux,uy,uz),
# and the normal (nx,ny,nz)
setDrawingPlane x0 y0 z0 ux uy uz nx ny nz

# Examples
#setDrawingPlane 0 0 0  1 0 0  0 0 1  # XY
#setDrawingPlane 0 0 0  1 0 0  0 1 0  # XZ
#setDrawingPlane 0 0 0  0 1 0  1 0 0  # YZ

# Set the drawing plane to a specific plane centered at the origin
setDrawingPlaneToXY
setDrawingPlaneToXZ
setDrawingPlaneToYZ

# Line is drawn from (x0,y0,z0) to (x1,y1,z1) on the drawing plane.
# Points must be legal for the drawing plane.
drawLine name x0 y0 z0 x1 y1 z1

# Polyline is drawn for the given N points on the drawing plane.
# Points must be legal for the drawing plane.
# The polyline is closed if x0=xN, y0=yN, z0=zN
drawPolyine name N x0 y0 z0 x1 y1 z1 ... xN yN zN

# Rectangle is drawn from (x0,y0,z0) to (x1,y1,z1) on the drawing plane.
# Points must be legal for the drawing plane.
drawRectangle name x0 y0 z0 x1 y1 z1

# Polycircle with center at (x0,y0,z0), first point at (x1,y1,z1).
# Points must be legal for the drawing plane.
drawPolycircle name x0 y0 z0 x1 y1 z1 

undo
redo
fitAll

# Operations on specific objects follows the method used when driving the user interface manually:
# clear the selection, select one or more objects, then select an operation to perform on the
# selected object.  A typical sequence is:
#    unselectAllObjects
#    selectObject name
#    deleteSelectedObjects

# Unselect all drawing items
unselectAllObjects

# Select a drawing item by name, where name is what appears in the selection tree.
# Only drawing items on the first level are selectable.
selectObject name

# Delete the drawing items and promote the children.
deleteSelectedObjects

# Delete the drawing item  and its children.
deleteSubtreeSelectedObjects

# Extrude the selected 2D drawing items by the given length and then re-name it to the given name.
extrudeSelectedObjects name length

# Merge the selected 3D drawing items and then re-name the result to the given name.
# Works only with two selected objects.
mergeSelectedObjects name

# Subtract the selected 3D drawing items and then re-name the result to the given name.
# Works only with two selected objects. Subtracts the second selected object from the first.
subtractSelectedObjects name

# Move the selected drawing items by the relative distance (x,y,z)
moveSelectedObjects x y z

# Copy the selected drawing item, where the name is copied and appended with "_copy"
copySelectedObjects

# Rotate the drawing items about the axis given by (x0,y0,z0) to (x1,y1,z1)
# by the given angle in degrees.
rotateSelectedObjects x0 y0 z0 x1 y1 z1 angle

# Conversion options
convertSelectedObjectsToPolylines
convertSelectedObjectsToPaths
convertSelectedObjectsToPorts
convertSelectedObjectsToBoundaries

# select the voltage or current object for the port with the given name
selectPortVoltageObject name
selectPortCurrentObject name

# Drawn lines and polylines for integration paths must be within the port outline
drawPortIntegrationLine x0 y0 z0 x1 y1 z1
drawPortIntegrationPolyline N x0 y0 z0 x1 y1 z1 ... xN yN zN

# Similar to drawing items, paths, ports, and boundaries are first selected then processed.

unselectAllPaths
unselectAllPorts
unselectAllBoundaries
...
# Setup variables can be changed with the following commands.
# This is a small subset of those available through the project file and the GUI of OpenParEMg.
# Errors in setting these will be reported when OpenParEM3D runs.

# none, high, low, highlow, lowhigh, all, plan
setRefinementFrequency text

# 0 for no normalization
setNormalizationZo Zo
...
addSimulationFrequency f

setLocalMaterialDatabase path filename
setGlobalMaterialDatabase path filename
setMaterialToObject name material
...
# simple scripts with no command line inputs
runScript name

# Simple looping for stress testing
# Note that there is no processing with i, so it is just a placeholder in the command.
For i = N To M Step P
   command
   command
   ...
