...
OpenParEM can be compiled for Linux-based operating systems and does not require administrator priviledges if the needed build tools make, cmake, g++, gfortran, perl, python3, tcl/tk, pkg-config, autoconf, and patchelf are already installed.  If cloning the project from GitHub, then autoconf is also required.  When compiling, OpenMPI is the default, but MPICH can be used instead with a command-line switch.
