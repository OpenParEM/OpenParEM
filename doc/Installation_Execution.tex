...
OpenParEM consists of the 3D simulator OpenParEM3D, its graphical user interface (GUI) front-end OpenParEMg, and the 2D simulator OpenParEM2D plus some helper tools.  All can be compiled from source or installed as pre-compiled bindaries for x86 architectures.

OpenParEMg provides a front-end for building 3D models, setting up 3D simulations, and executing simulations using OpenParEM3D and OpenParEM2D in the background.  OpenParEMg does 
not support plotting or graphing, so external tools are required for these tasks.  For plotting S-parameters, any of the free S-parameter plotting tools can be used, or charts can be built in spreadsheet tools.  For plots of electromagnetic fields, output is provided for ParaView \cite{paraview}.  ParaView, spreadsheets, or other post-processing tools must be installed separately.

OpenParEMg is not required to run OpenParEM3D.  The necessary files to run OpenParEM3D (and OpenParEM2D) can be built from other tools, such as with FreeCAD \cite{freecad} for 3D modeling, gmsh \cite{gmsh}\cite{gmshweb} for meshing, scripts for modes and ports, and hand-edited project control files.  Examples of projects built using open-source tools other than OpenParEMg are provided in the tutorials and regression test cases.

\section{Installing Pre-compiled Binaries}

Pre-compiled binaries are available for x86 Linux and do not require administrator priviledges to install.
...
\noindent The pre-compiled binaries are compiled with OpenMPI.

\section{Compiling from Source}

OpenParEM can be compiled for Linux-based operating systems and does not require administrator priviledges if the needed build tools make, g++, gfortran, perl, python3, pkg-config, autoconf, automake, libtool, patchelf, and shasum are already installed.  When compiling, OpenMPI is the default, but MPICH can be used instead with a command-line switch.  Note that when installing libtool, two packages are required to be installed: libtool and libtool-bin.  The build script \texttt{OpenParEM_build} checks for the availability of the required tools, and if any are missing, then it lists the needed tools and exits.  Any missing tools should be installed with the operating system's package manager, then \texttt{OpenParEM_build} can be re-run.  If all required tools are found, then the script proceeds.

\subsection{Ubuntu 22}

The library \texttt{xkeyboard-config} requires Python 3.11, but due to its age, Ubuntu 22 uses Python 3.10.  To compile OpenParEM in Ubuntu 22 requires using Python 3.11 for compiling \texttt{xkeyboard-confg}.  Python 3.11 must be installed on the system, but it must no be installed for general system use to keep from breaking other applications.  The OpenParEM build script uses the installed Python 3.11 version in a virtual environment to avoid installing it for general use.

To install Python 3.11, issue the following command (requires root priviledge):
\begin{itemize}[nosep]
   \item $ sudo apt install software-properties-common
   \item $ sudo add-apt-repository ppa:deadsnakes/ppa
   \item $ sudo apt update
   \item $ sudo apt install python3.11 python3.11-dev python3.11-venv
\end{itemize}

During execution, \texttt{OpenParEM_build} checks for the presence of Python 3.10, and if it finds it, it checks for the availability of python3.11.  If it finds it, processing continues, otherwise an error is printed.  Note that this simple check is not intended to cover any of the many possible corner cases: it is just here to get Ubuntu 22 to compile.

\subsection{Using a Downloaded Package}

For installing using a downloaded installation package, the basic steps for installation are listed below.  See the INSTALL file for more details and options.
\begin{itemize}[nosep]
...
$ ./OpenParEM_build --jobs=4 --with-all
         \end{Verbatim}
   \item Add the command-line option --with-mpic to use MPICH instead of OpenMPI.
...
For installing using using a cloned package from GitHub, the basic steps for installation are listed below.  See the INSTALL file for more details and options.
\begin{itemize}[nosep]
   \item Clone the project from GitHub.  This step requires that git is installed on the system.
         \begin{Verbatim}
$ git clone https://github.com/OpenParEM/OpenParEM
         \end{Verbatim}
   \item Change into the base directory and view the installation instructions:
         \begin{Verbatim}
$ cd OpenParEM
$ more INSTALL
         \end{Verbatim}
   \item Install make, g++, gfortran, perl, and/or autoconf as needed.
   \item Change into the scripts directory and compile:
         \begin{Verbatim}
$ cd scripts
$ ./OpenParEM_build --jobs=4 --with-all
...
See the INSTALL file for uninstallation instructions.

\section{OpenParEMg}

OpenParEMg is a front-end GUI for OpenParEM3D, supporting 3D geometry creation, meshing, simulation setup, and simulation execution.  It does not support graphing or plotting since very capable options for these functions already exist as free open-source software.  User's will typically install ParaView (see below) for field plots and either a spreadsheet or S-parameter plotting tool for making S-parameter plots.

\section{Flow Tools}

OpenParEM3D and OpenParEM2D are command-line tools for running electromagnetic simulations only.  Pre- and post-processing must be handled by other tools.  For pre-processing, OpenParEMg provides a complete front-end.  If not using OpenParEMg, then FreeCAD and gmsh along with handy scripts can implement a usable front-end, and their use is described in the tutorials. Many other tools exist that can be used to build a front-end process.  For post-processing, ParaView is supported for plotting fields.
...
\begin{verbatim}
   $ proj_search has fem.order | grep 6
\end{verbatim}
...
OpenParEMg is simply started at the command line as
\begin{verbatim}
   $ OpenParEMg
\end{verbatim}
Some information is scrolled to the starting terminal, particularly from gmsh, and this can be captured into a log file, if desired, with
\begin{verbatim}
   $ OpenParEMg >& OpenParEMg.log
\end{verbatim}
\noindent Execution of OpenParEM3D and OpenParEM2D are completely managed by OpenParEMg.  Even when a project is set up in OpenParEMg, it is always possible to run OpenParEM3D manually as described next.
...
When parallel processing, the \verb+-np+ option specifies the number of cores to use for the simulation, given by \verb+N+.  The \verb+-q+ option suppresses messages from the MPI infrastructure, which are rarely needed.  The \verb+--oversubscribe+ is required when using OpenMPI and when \verb+N+ is more than half the number of available cores.

\section{Specifying Core Counts}

OpenParEM3D and OpenParEM2D can be run using parallel processing to reduce run times, and the user control for this is the number of cores specified for the simulation.  The number of cores is set in OpenParEMg in the \texttt{Simulation Setup/Options ...} window, and when running at the command line, by the \verb+-np+ command line option to \texttt{mpirun}. 

The number of cores to specify requires some consideration.  For simulations on a single computer, \verb+N+ should always be set to equal to or less than the number of physical cores.  For high-performance computing, there is not sufficient left-over capacity for threads to be utilized.  Depending on the computer, setting \verb+N+ to the number of physical cores can (and probably will) run slower than if \verb+N+ is set to a lower number.  The reason is that the available bandwidth on the CPU itself and to/from memory becomes saturated, so adding more cores causes MPI messages to be delayed due to limited bandwidth, slowing overall performance.  Experimenting with the value of \verb+N+ can identify the value that produces the fastest run times for a given computer.  Different computers have different optimal values of \verb+N+.  See Sec.~\ref{sec:computer} for additional discussion.

When running OpenParEM2D with \verb+N+ cores, \verb+N+ copies of OpenParEM2D are run in parallel with one instance per core.  When running OpenParEM3D with \verb+N+ cores, \verb+N+ copies of OpenParEM3D are run with one instance per core, \textit{plus} \verb+N+ copies of OpenParEM2D are run with one instance per core while ports are being solved.  For some times during the simulation, 2*\verb+N+ cores are being utilized.  If running MPI with OpenMPI and if 2\verb+N+~\textgreater~the number of physical cores, then OpenMPI issues an error and exits.  Specifying \verb+--oversubscribe+ allows processing to continue without errors.  OpenParEM3D sleeps while OpenParEM2D runs, so the actual load on the computer is only slightly more than \verb+N+.
