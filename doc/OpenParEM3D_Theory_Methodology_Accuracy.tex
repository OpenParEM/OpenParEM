...
Convergence testing relies on the absolute error calculated on $\overline{H}$ using \texttt{fem3D::calculateMeshErrors} and on the relative error calculated on the S-parameter matrices $\overline{\overline{S}}_N$ and $\overline{\overline{S}}_{N-1}$ from the $N^{\textnormal{th}}$ and $(N-1)^{\textnormal{th}}$ iterations using the Frobenius norm as the error metric
\begin{equation}
\text{error}=\frac{\|\overline{\overline{S}}_N-\overline{\overline{S}}_{N-1}\|}{\|\overline{\overline{S}}\|}
\end{equation}
