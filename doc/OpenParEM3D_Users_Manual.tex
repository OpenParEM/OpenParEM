...
OpenParEM3D is a command-line tool that does just the electromagnetic calculations.  To complete a project, other tools must be used to create and mesh a geometry and plot results.  A graphical user interface (GUI) called OpenParEMg (see \"OpenParEMg\_Users\_Manual.pdf") provides a complete front-end for OpenParEMg enabling the drawing and meshing of 3D structures, assignments of ports and boundary conditions, simulation setup, and simulation execution and monitoring.  Post-processing is handled by ParaView \cite{ParaView} for fields and by the user's selection of spreadsheets or S-parameter plotting tools for S-parameters.  Also documented in this guide is a front-end simulation flow based on FreeCAD \cite{FreeCAD} and gmsh \cite{gmsh}\cite{gmshweb} along with command-line execution and monitoring.  Any number of alternative tool collections can be used as front-ends and back-ends for OpenParEM3D..
...
\begin{itemize}[nosep]
  ...
  \item Finite element order ramping for 2D solves in OpenParEM2D
  \item Front-end graphical user interface OpenParEMg
\end{itemize}
\noindent \hrulefill
...
OpenParEMg is a GUI front-end for OpenParEM3D to enable the creation and execution of projects for OpenParEM3D.  All needed files are managed by OpenParEMg, so the user does not need to be familiar with the file formats described below.  All OpenParEM3D files are human-readable text with simple formats, so familiarity of the file formats does open up opportunities to modify or create setups by script for enhanced productivity.  Note that visualization of fields and S-parameters require external tools.

Since OpenParEMg programmatically runs OpenParEM3D using the command line, projects set up by OpenParEMg can always be run manually at the commmand line.  In other words, OpenParEMg and command line execution run the same compiled executable for OpenParEM3D.
...
\end{Verbatim}
where \verb+N+ is the number of cores to use for the calculation.  All inputs are captured in the project control file including names of files that must be included. So setting up a project involves creating/editing a project control file and ensuring that the additionally required files are specified and available.  Note that the \texttt{--oversubscribe} option is used with OpenMPI\cite{OpenMPI} and is omitted for MPICH\cite{MPICH}.  
When using OpenParEMg, the project file includes GUI-specific data that is ignored when executing from the command line.

The execution output from OpenParEM3D is scrolled to the terminal.  To create a log file, the output can be re-directed.  When OpenParEMg runs OpenParEM3D, the output that normally scrolls to the terminal is captured to a log file with the name \texttt{projectname.log} for display in the GUI.

The required included files in a project control file are materials, mesh, and port definitions. Each of the required files are covered in detail in the following sections.  When using OpenParEMg, these additional files are handled automatically.
...
\noindent Additional keyword/value pairs can be added to the control file to do things like outputting files to enable plotting with ParaView \cite{ParaView} [project.save.fields true] or to turn on adaptive mesh refinement [ex. refinement.frequency high].
...
Materials are specified in one or two materials files following the specification in Appendix~\ref{sec:materials_file_spec}.  At least one materials file is required to run OpenParEM3D, using either \texttt{materials.global.path} plus \newline \texttt{materials.global.name} or \texttt{materials.local.path} plus \texttt{materials.local.name}.  Typical simulation environments will have the bulk of the materials specified in a global file for general use across all projects, then a local materials file for materials specialized for the current project.

The material type is either \"dielectric\" or \"conductor\" and is only used in GUIs to reduce the material selection to appropriate types.  The material type never plays a role in a calculation.
...
When using AMR, the refined mesh can be output using the keyword/value pair \texttt{mesh.save.refined true}.  At each iteration, the mesh is output and can be viewed using gmsh to visually check the progress of the mesh adaptation.

\subsection{Port Definition File}
\label{sec:port_definition_file}

Ports and boundaries are specified in a text file following the specification in Appendix~\ref{sec:port_file_spec}.  The project control file specifies the port definition file using the \texttt{port.definition.file} keyword. Due to complexity, the port definition file is very difficult to write by hand, so scripted generation is generally required.  When using OpenParEMg, the file is created automatically.  When using FreeCAD in a manual setup, the Python script "OpenParEM3D\_save.py" can be used to generate the port definition file.
...
An automated regression suite is in place as part of the release methodology. The projects with short descriptions are listed in Appendix~\ref{sec:regression_suite}.  The suite also represents a large number of worked examples that are ready to run with known answers.  For each project, there are two implementations: OpenParEMg, and a manual setup using FreeCAD and gmsh.

To run the project using OpenParEMg, simply start OpenParEMg and open the project file with the \texttt{proj} extension, then execute the project by clicking the \texttt{Run} menu item.  After successful completion, the results can be checked by executing the command line script \texttt{./check\_results.sh} in a terminal directed to the project directory.  The project can also be run manually as described next for the manual setup.

The manual setup is in the project subdirectory \texttt{freecad\_build}, so in a terminal pointed at this subdirectory, the project can be executed at the command line with
\begin{Verbatim}[fontsize=\small]
   $ mpirun -np 12 --oversubscribe OpenParEM3D project.proj
\end{Verbatim}
where \texttt{12} is the number of processors to use appropriate for the computer being used, \texttt{--oversubscribe} is for OpenMPI and omitted for MPICH, and \texttt{project.proj} is the project file name for the given project.  Like for OpenParEMg, after successful completion, the results can be checked by executing the command line script \texttt{./check\_results.sh}.  The log file can be viewed during execution from another terminal using the \texttt{tail -f} command as previously described.

For example, for the regression project \verb+WR90/straight/freecad_build/straight.proj+, at the command line in that project directory enter
\begin{Verbatim}[fontsize=\small]
   $ mpirun -np 12 --oversubscribe OpenParEM3D straight.proj
\end{Verbatim}
where depending on the computer, a number smaller or larger than 12 could be appropriate for the number of cores to use, and OpenMPI is assumed.  After successful completeion, run
B\begin{Verbatim}[fontsize=\small]
   $ ./check\_results.sh
\end{Verbatim}
which prints the results comparing the computed results to the expected results.  Small errors labeled as \texttt{FAIL} can be ignored.  Additional files are produced including the S-parameters in the csv file "straight\_results.csv".  Since this example does not renormalize the S-parameters, a Touchstone file is not produced.  See Sec.~\ref{sec:Soutput} for more details.  To view the fields, set \texttt{project.save.fields true} before running the project.  Then run ParaView to setup and view fields.
The regression suite cases are generally set up for more accuracy than that needed for engineering work to enable tight pass/fail criteria.  In many situations, looser settings are appropriate.
...
The complete drawing, setup, and execution of an OpenParEM3D project can be managed using OpenParEMg, which has its own dedicated user document "OpenParEMg\_Users\_Manual.pdf".  However, OpenParEMg is not required and any number of other tools can be used to execute simulations.

The process of manually setting up and running a project from scratch involves several steps: draw the geometry via CAD or script, define ports and set boundary conditions [if needed], create a local materials file if the needed materials do not exist in the global library file, and finally run OpenParEM3D.  Except for the OpenParEM3D step, any number of tools and techniques can be used to generate the needed files.  One flow is described in detail here that can be used directly or used as a guideline for setting up a flow using different tools.
...
there are three continuously updated files showing progress: \texttt{projectname.log}, \texttt{projectname\_results.csv}, and \texttt{projectname\_iterations.csv}.  They can be viewed as scrolling log files by opening three new terminals, then after the files appear, run
\begin{Verbatim}[fontsize=\small]
   $ tail -f projectame.log
\end{Verbatim}
\noindent in one terminal,
\begin{Verbatim}[fontsize=\small]
   $ tail -f projectame_results.csv
\end{Verbatim}
\noindent in a second terminal, and
\begin{Verbatim}[fontsize=\small]
   $ tail -f projectame_iterations.txt
\end{Verbatim}
\noindent in a third terminal.  The three terminals together enable a comprehensive view of execution progress.
...
Several projects are created from scratch to demonstrate the setup and use of OpenParEM3D.  The tutorials are covered twice: once for OpenParEMg and once with a manual setup based on FreeCAD and gmsh.  The tuturals using OpenparEMg are covered in the dedicated OpenParEMg user manual "OpenParEMg\_Users\_Manual.pdf".  The tutorials using the manual setup are covered in detail below.
...
\begin{figure}
  \centering
  \includegraphics[width=0.75\textwidth]{../tutorials/OpenParEM3D/rectangular_waveguide/freecad/screenshots/rectangle}
...
\begin{figure}
  \centering
  \includegraphics[width=0.75\textwidth]{../tutorials/OpenParEM3D/rectangular_waveguide/freecad/screenshots/extrusion}
...
\begin{figure}
  \centering
  \includegraphics[width=0.75\textwidth]{../tutorials/OpenParEM3D/rectangular_waveguide/freecad/screenshots/portout-crop}
...
\begin{figure}
  \centering
  \includegraphics[width=0.75\textwidth]{../tutorials/OpenParEM3D/rectangular_waveguide/freecad/screenshots/hover}
...
\begin{figure}
  \centering
  \includegraphics[width=0.75\textwidth]{../tutorials/OpenParEM3D/rectangular_waveguide/freecad/screenshots/lock-crop}
...
\begin{figure}
  \centering
  \includegraphics[width=0.75\textwidth]{../tutorials/OpenParEM3D/rectangular_waveguide/freecad/screenshots/drawing_finished}
...
\begin{figure}
  \centering
  \includegraphics[width=0.75\textwidth]{../tutorials/OpenParEM3D/rectangular_waveguide/freecad/screenshots/mesh}
...
\end{Verbatim}
to run in parallel with 12 cores.  Substitute a larger or smaller core number as needed.  The option \verb+--oversubscribe+ is not needed if the number of cores is less than or equal to half the number of available cores.  The log output can be viewed as previously described using \texttt{tail -f rectangular\_waveguide.proj}, although this run is so short that it is not really needed.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\textwidth]{../tutorials/OpenParEM3D/rectangular_waveguide/freecad/screenshots/ReEy}
...
\begin{figure}[H]
  ...
  \begin{subfigure}{0.73\textwidth}
     \includegraphics[width=\linewidth]{../tutorials/OpenParEM3D/rectangular_waveguide/freecad/screenshots/ReE} 
     \caption{Re($\overline{E}$)}
  \end{subfigure}
  \par\bigskip
  \begin{subfigure}{0.73\textwidth}
     \includegraphics[width=\linewidth]{../tutorials/OpenParEM3D/rectangular_waveguide/freecad/screenshots/ReH}
...
\begin{figure}
  \centering
  \includegraphics[width=0.75\textwidth]{../tutorials/OpenParEM3D/microstrip_step/freecad/screenshots/microstrip2D}
...
\begin{figure}
  \centering
  \includegraphics[width=0.75\textwidth]{../tutorials/OpenParEM3D/microstrip_step/freecad/screenshots/microstrip_extruded}
...
\begin{figure}
  \centering
  \includegraphics[width=0.75\textwidth]{../tutorials/OpenParEM3D/microstrip_step/freecad/screenshots/microstrip3D}
...
\begin{figure}
  \centering
  \includegraphics[width=0.75\textwidth]{../tutorials/OpenParEM3D/microstrip_step/freecad/screenshots/microstrip_mesh}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\textwidth]{../tutorials/OpenParEM3D/microstrip_step/freecad/screenshots/microstrip_ReEy}
...
\begin{figure}
  ...
  \begin{subfigure}{0.75\textwidth}
     \includegraphics[width=\linewidth]{../tutorials/OpenParEM3D/monopole_antenna/freecad/screenshots/monopole_building_blocks}
     \caption{View of all}
  \end{subfigure}
  \par\bigskip
  \begin{subfigure}{0.75\textwidth}
     \includegraphics[width=\linewidth]{../tutorials/OpenParEM3D/monopole_antenna/freecad/screenshots/monopole_zoom}
...
\begin{figure}[H]
  ...
  \begin{subfigure}{0.5\textwidth}
     \includegraphics[width=\linewidth]{../tutorials/OpenParEM3D/monopole_antenna/freecad/screenshots/monopole_mag}
     \caption{Magnitude.}
  \end{subfigure}
  \par\bigskip
  \begin{subfigure}{0.5\textwidth}
     \includegraphics[width=\linewidth]{../tutorials/OpenParEM3D/monopole_antenna/freecad/screenshots/monopole_phase}
     \caption{Phase.}
  \end{subfigure}
...
\begin{figure}[H]
  ...
  \begin{subfigure}{0.33\textwidth}
     \includegraphics[width=\linewidth]{../tutorials/OpenParEM3D/monopole_antenna/freecad/screenshots/monopole_3D_gain}
     \caption{Isotropic gain.}
  \end{subfigure}
  \begin{subfigure}{0.33\textwidth}
     \includegraphics[width=\linewidth]{../tutorials/OpenParEM3D/monopole_antenna/freecad/screenshots/monopole_3D_gain_slices}
...
\begin{figure}[H]
  ...
  \begin{subfigure}{0.408\textwidth}
     \includegraphics[width=\linewidth]{../tutorials/OpenParEM3D/monopole_antenna/freecad/screenshots/radiation_pattern_G_4.75e+09_xz_0_180_SP1}
     \caption{Slice through the x-z plane.}
  \end{subfigure}
  \begin{subfigure}{0.41\textwidth}
     \includegraphics[width=\linewidth]{../tutorials/OpenParEM3D/monopole_antenna/freecad/screenshots/radiation_pattern_G_4.75e+09_xy_15_0_SP1}
...
\begin{figure}[H]
  ...
  \begin{subfigure}{0.6\textwidth}
     \includegraphics[width=\linewidth]{../tutorials/OpenParEM3D/monopole_antenna/freecad/screenshots/Eplane_4p75GHz}
     \caption{$|\textnormal{Re}(\overline{E})|$}
  \end{subfigure}
  \par\bigskip
  \begin{subfigure}{0.6\textwidth}
     \includegraphics[width=\linewidth]{../tutorials/OpenParEM3D/monopole_antenna/freecad/screenshots/Eplane_vector_4p75GHz}
     \caption{$\textnormal{Re}(\overline{E})$}
  \end{subfigure}
...
\begin{figure}[H]
  \centering
  \begin{subfigure}{0.6\textwidth}
     \includegraphics[width=\linewidth]{../tutorials/OpenParEM3D/monopole_antenna/freecad/screenshots/Hplane_4p75GHz}
     \caption{$|\textnormal{Re}(\overline{H})|$}
  \end{subfigure}
  \par\bigskip
  \begin{subfigure}{0.6\textwidth}
     \includegraphics[width=\linewidth]{../tutorials/OpenParEM3D/monopole_antenna/freecad/screenshots/Hplane_vector_4p75GHz}
     \caption{$\textnormal{Re}(\overline{H})$}
  \end{subfigure}
...
The math behind OpenParEM3D assumes that the 3D electromagnetic fields at the ports represent transmission lines or waveguides, meaning that the field dependence along the direction orthogonal to the port ($\hat{n}$) has the dependence $e^{-\gamma n}$.  Ports must be placed on the ends of transmission lines or waveguides that are long enough to ensure that the electromagnetic fields have settled into their modal configurations.  How long is long enough?  A reasonable rule-of-thumb is to make the transmission lines or waveguides twice as long as the lateral extent of the fields, which may or may not align with the dimensions of the port.  For critical work, it could be reasonable to run the problem a second time with a longer transmission lines or waveguides to see if the S-parameters change other than the phase shift due to the longer feeds.  Visual inspection of the fields from a completed solution can also help determine if the transmission lines or waveguides for the ports are long enough.  \textit{Failure to ensure that the port transmission lines or waveguides are long enough will result in incorrect answers with no warning from OpenParEM3D.}
...
The finite element order is set with the keyword/value pair \texttt{fem.order N} in the project control file.  \texttt{N} can be any integer and sets the order of the polynomial approximating the field in each finite element.  All elements in a mesh have the same element order.
...
\item Harder convergence in 2D simulations, although order ramping in OpenParEM2D helps with this problem.
\end{itemize}
...
\subsection{Lossy Conductors Touching Ports}

The 2D waveport solutions calculated with OpenParEM2D take conductor losses into account via a perturbational approach. The 2D problem is solved including dielectric losses but with lossless conductors, then the computed fields are used to calculate the conductor loss which are then added to the dielectric loss to get the total loss.  This means that the computed field structure at the conductor boundaries are those of the lossless conductors.

OpenParEM3D calculates the fields for the lossy conductors directly, so the field structure at the conductor boundaries are those of the lossy conductors.  In imposing the 2D field structure onto the 3D space, the lossless fields are imposed, and OpenParEM3D will generate small parasitics to align the field structure from the lossless conductors at the prots to the lossy conductors in the 3D structure.  It is up to the user to ensure that these parasitics are negligible for the problem at hand.

In the regression suite and accuracy tests for OpenParEM3D, the parasitics are assumed neglible in all cases except two: \texttt{WR90/straight-lossy-default} and \texttt{partially\_filled\_rect\_waveguide/partially\_filled\_rect\_waveguide\_lossy}.  In these cases, the surface impedance boundary condition is pulled back slightly from the port so that the 2D port fields, which use a PEC boundary, touch a PEC boundary in the 3D space.  The transition from PEC to surfance impedance boundary is then handled fully in OpenParEM3D.
...
OpenParEMg automatically implements these two techniques.
...
Much of the volume that is meshed and solved for an antenna is the freespace between the antenna and the radiation boundary condition.  Since OpenParEM3D has a 1$^{\textnormal{st}}$-order radiation boundary condition, the boundary must be placed about 1$\lambda$ or more from the antenna at the lowest simulation freqeuncy to minimize reflections.  The field must be well-modeled throughout the volume to avoid reflections from poorly modeled regions.  In other words, the fields must be well-resolved throughout the 3D volume.
...
The first step in solving a 3D problem is to solve the 2D eigenvalue problems for the fields and propagation constants at the wave ports using OpenParEM2D.  Since these calculations are full-wave electromagnetic simulations, they can become problematic if they are failing to converge since that stalls the 3D simulation. To help with 2D convergence, OpenParEM2D implements a technique called \textit{order ramping}, where a 2D port is solved with finite elements of order 1, and the solution is used as the initial guess for a solve with order 2, which is then used as the initial guess for order 3, etc., until the user-specified order for the problem is reached.  Order ramping takes advantage of the fact that solves with first order elements are far easier numerically and are far more likely to quickly converge than higher-order elements.  With a good initial guess, the higher-order element solves then proceed quickly.

Order ramping is the default, but it can be turned off if needed using the keyword/value pair \newline\texttt{solution.initial.guess.level value}.  See "OpenParEM2D\_Users\_Manual.pdf" for details.

Scenarios leading to issues with 2D wave port convergence are described in the following sections.
...
\begin{longtable}[c]{|lccp{8cm}|}
    ...
    mesh.order                   & int    & 1     & Deprecated for fem.order \\
    ...
    fem.order                    & int    & 1     & Order of the finite elements \\
    ...
    refinement.frequency         & string & high & Sets the frequency or frequencies on which adaptive mesh refinement is applied.
    ...
    solution.initial.guess.level  & int    & 2      & Sets the initial guess method for OpenParEM2D. Default is to use order ramping \\
    ...
    antenna.plot.2D.pattern       & string & none   & Defines a plane that cuts a slice through the 3D antenna pattern and the quantity or quantities to plot on that plane. The basic cut plane is the x-y plane through z=0.  The plane can be reoriented by providing values for the spherical coordinate angles theta and phi.  Phi is the angle in the x-y plane from the x-axis, and theta is the angle from the z-axis.  For convenience, theta and phi can be specified by substituting the named planes xy, xz, or yz.  The plane can be shifted from the origin using the optional latitude, where the direction of movement is normal to the plane. \\
                                  ...
                                  &        &        &    G and D cannot plot with a field quantity\\
                                  &        &        &    \\
    ...
    test.show.detailed.cases      & bool   & false  & Show detailed information about the test cases when regression testing with the program called “process3D”.  This keyword is not used by OpenParEM2D \\
    ...
    gui.slot.count                & int    & 5      & Only used for the GUI OpenParEMg. Number of slots to use when running OpenParEM3D.  Used on mpirun as mpirun -np gui.slot.count \\
    gui.oversubscribe             & bool   & true   & Only used for the GUI OpenParEMg. Use the oversubscribe keyword when running MPI jobs.  Only for OpenMPI.  Uncheck for MPICH. \\
    gui.units                     & text   & mm     & Only used for the GUI OpenParEMg. Length unit for the various forms: m, mm, cm, um, ft, in, mil. \\
    gui.grid.size                 & double & 10     & Only used for the GUI OpenParEMg. Size of the drawing grid as a multiple of the units set with gui.units. \\ 
    gui.mesh.scale                & double & 1      & Only used for the GUI OpenParEMg. Scale factor elements in a generated mesh. \\
    gui.mesh.minSize              & double & 0      & Only used for the GUI OpenParEMg. Minimum allowed mesh element size.  Depends on mesh generation behavior. \\
    gui.mesh.maxSize              & double & 1e19   & Only used for the GUI OpenParEMg. Maximum allow mesh element size.  Depends on mesh generation behavior. \\
...
Dielectrics can be specified with a Debye model or by properties at a single frequency or a list of frequencies.  Conductors are specified by properties at a single frequency or a list of frequencies.  The material type is specified as either a dielectric or conductor to enable GUIs to customize the material selection to appropriate material types.  The material type is never used for calculations.
...
// Paths are required for calculating S-parameters.
...
Path
   ...
   [normal=(double,double,double)]   // optional normal for the path so that integration
                                     // directions can be drawn with oriented arrow heads
EndPath
...
\bibitem{OpenMPI} \verb+https://www.open-mpi.org/+

\bibitem{MPICH} \verb+https://www.mpich.org/+
