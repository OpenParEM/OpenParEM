...
void OpenParEMg::showDrawingItems ()
{
    ...
    while (i < ui->drawingWindow->get_selectedItems_size()) {
        ...
        if (item) {
            DrawingItem *drawingItem=dynamic_cast<DrawingItem *>(item);
            if (drawingItem) ui->drawingWindow->showItem(drawingItem);
        }
        i++;
    }
...
void OpenParEMg::showRootPathItems ()
{
    ...
    while (i < ui->drawingWindow->get_selectedItems_size()) {
        ...
        if (item) {
            ...
            if (rootPathItem) ui->drawingWindow->showItem(rootPathItem);
        }
        i++;
    }
...
void OpenParEMg::showPathItems ()
{
    ...
    while (i < ui->drawingWindow->get_selectedItems_size()) {
        ...
        if (item) {
            PathItem *pathItem=dynamic_cast<PathItem *>(item);
            if (pathItem) ui->drawingWindow->showItem(item);
        }
        i++;
    }
...
void OpenParEMg::hidePathItems ()
{
    ...
    while (i < ui->drawingWindow->get_selectedItems_size()) {
        ...
        if (item) {
            PathItem *pathItem=dynamic_cast<PathItem *>(item);
            if (pathItem) ui->drawingWindow->hideItem(pathItem);
        }
        i++;
    }
...
void OpenParEMg::deleteBoundaryItem (CustomTreeWidgetItem * item)
{
    //std::cout << "OpenParEMg::deleteBoundaryItem" << std::endl; std::cout.flush();

    // unlink the outline
    Boundary *boundary=(Boundary *)item->get_OPEMobject();
...
void OpenParEMg::insertPath (CustomTreeWidgetItem *item)
{
    ...
    while (i < ui->drawingWindow->get_selectedItems_size()) {
        ...
        if (selectedItem) {
            ...
            if (selectedItem) {
                Path *aPath=static_cast<Path *>(selectedItem->get_OPEMobject());
                if (aPath) {
    ...
    if (originalText.compare(newText) != 0) {
        ...
        if (pathItem) {
            ...
            while (i < pathItem->linkedItems_size()) {
                CustomTreeWidgetItem *item=pathItem->get_linkedItem(i);
        ...
        if (renameItem->is_boundary()) {
            Boundary *boundary=(Boundary *)renameItem->get_OPEMobject();
...
void OpenParEMg::unselectRootDrawingItems()
{
    ...
    while (i < ui->drawingWindow->get_selectedItems_size()) {
        ...
        if (item) {
            ...
            if (rootDrawingItem) ui->drawingWindow->unselectItem(item,i);
        }
        i++;
    }
                ...
                if (grandChild->is_voltage() || grandChild->is_current()) {
                    ...
                    while (k < grandChild->childCount()) {
                        ...
                        if (greatGrandChild->is_integrationPathSegment()) {
                            ...
                            // unlink
                            Path *aPath=static_cast<Path *>(greatGrandChild->get_OPEMobject());
                            CustomTreeWidgetItem *pathItem=aPath->get_item();
        ...
        if (child->is_voltage() || child->is_current()) {
            ...
            while (j < child->childCount()) {
                ...
                if (grandChild->is_integrationPathSegment()) {
                    ...
                    // unlink
                    Path *aPath=static_cast<Path *>(grandChild->get_OPEMobject());
                    CustomTreeWidgetItem *pathItem=aPath->get_item();
    ...
    while (i < ui->drawingWindow->get_selectedItems_size()) {
        ...
        if (item) {
            ...
            if (drawingItem) {
                ...
                if (parent) {
                    RootDrawingItem *rootDrawing=dynamic_cast<RootDrawingItem *>(parent);
                    if (rootDrawing) {
...
void OpenParEMg::finishMoveObject (gp_Pnt p0, gp_Pnt p1)
{
    ...
    while (i < ui->drawingWindow->get_selectedItems_size()) {
        ...
        if (item) {
            DrawingItem *drawingItem=dynamic_cast<DrawingItem *>(item);
            std::cout << "*** finish move" << std::endl; std::cout.flush();
...
void OpenParEMg::cancelDeletePoint ()
{
    ...
    while (i < ui->drawingWindow->get_selectedItems_size()) {
        ...
        if (item) {
            DrawingItem *drawingItem=dynamic_cast<DrawingItem *>(item);
            if (drawingItem) drawingItem->cancelDeletePoint();
        }
        i++;
    }
...
void OpenParEMg::insertPoint ()
{
    ...
    while (i < ui->drawingWindow->get_selectedItems_size()) {
        ...
        if (item) {
            DrawingItem *drawingItem=dynamic_cast<DrawingItem *>(item);
            if (drawingItem) drawingItem->startInsertPoint();
        }
        i++;
    }
}
...
void OpenParEMg::cancelInsertPoint ()
{
    ...
    while (i < ui->drawingWindow->get_selectedItems_size()) {
        ...
        if (item) {
            DrawingItem *drawingItem=dynamic_cast<DrawingItem *>(item);
            if (drawingItem) drawingItem->cancelInsertPoint();
        }
        i++;
    }
    finishOperation(true,4005);
...
//xxx
void OpenParEMg::convertItemToPath (CustomTreeWidgetItem *item, bool saveForUndoRedo)
{
    ...
    if (newPathItem) {
        ...
        newShapeData->setShape(polywire->get_AIS_Shape());
            ...
            if (pathItem) {
                ...
                if (pathItem->linkedItems_size() > 0) {
                    ...
                    while (j < pathItem->linkedItems_size()) {
                        CustomTreeWidgetItem *linkedItem=pathItem->get_linkedItem(j);
...
void OpenParEMg::reversePathItems ()
{
    ...
    while (i < ui->drawingWindow->get_selectedItems_size()) {
        ...
        if (item) {
            PathItem *pathItem=dynamic_cast<PathItem *>(item);
            if (pathItem) changeList.push_back(item);
        }
        i++;
    }
    ...
    if (QFile::exists(projectFile)) {
        ...
        if (strcmp(projData.port_definition_file,"") != 0) {
            ...
            while (i < boundaryDatabase->get_pathList_size()) {
                ...
                CustomTreeWidgetItem *item=aPath->get_item();
                if (item) insertToMapActivateItem(item);
                i++;
            }
        }
                           ...
                           long unsigned int &endBlockIndex, CustomTreeWidgetItem *parent, bool increaseDepth)
{
    ...
    if (polywire) {
        ...
        if (drawingItem) newItem->copy_depth(parent);
...
void OpenParEMg::on_drawingItemTree_itemClicked (QTreeWidgetItem *item, int column)
{
    ...
    clickedItem=dynamic_cast<CustomTreeWidgetItem *>(item);
    if (!clickedItem) return;
...
void OpenParEMg::undoItem (CustomTreeWidgetItem *item)
{
    ...
    DrawingItem *drawingItem=dynamic_cast<DrawingItem *>(item);
    if (drawingItem) drawingItem->undo();
}
...
void OpenParEMg::redoItem (CustomTreeWidgetItem *item)
{
    ...
    DrawingItem *drawingItem=dynamic_cast<DrawingItem *>(item);
    if (drawingItem) drawingItem->redo();
}
